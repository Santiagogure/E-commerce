...
const CardItem = ({ product, showAction }) => {
  ...
  return (
    <div
      className={`flex  max-h-[250px] lg:max-h-[450px] ${
        showAction
          ? "min-h-[10rem] w-48 shadow-lg lg:w-72 "
    ...
    >
      {showAction ? (
        <>
          ...
          >
            <img
              className="h-full max-h-[150px] w-full max-w-[150px] transform object-cover transition-all duration-200 ease-in-out hover:-translate-y-1 hover:scale-110 lg:max-h-[350px] lg:max-w-[350px]"
              src={product.image}
              alt={product.name}
            ></img>
          </Link>
          <div className="flex-col items-center justify-center py-5">
            <div className="flex flex-col items-center justify-center">
              <h2 className="mt-2 max-w-[80%] overflow-hidden overflow-ellipsis whitespace-nowrap text-[1.3rem] font-semibold">
                {product.name}
              </h2>
